  ...
  return (
    <Show when={paytableOpen()}>
      <dialog
        ...
        onClose={close}
        onClick={(e) => {
          if (e.target.id === 'paytableModal') close();
        }}
        onKeyDown={(e) => {
      ...
      >
        <div class="modal-content" style="max-width:560px;">
          <div class="modal-header">
            ...
            <button id="closePaytableBtn" class="btn-ghost" onClick={close}>
              ×
            </button>
          </div>
          <div
            ...
            <Show when={game().paytable && Object.keys(game().paytable || {}).length > 0}>
              <For each={Object.entries(game().paytable || {})}>
                {([id, rule]) => {
                  ...
                  const name =
                    game().symbols?.[id] !== undefined ? game().symbols[id] : `Symbol ${id}`;
                    ...
                    <div style="display:flex; align-items:center; justify-content:space-between; background:rgba(255,255,255,0.03); padding:12px; border-radius:8px; border:1px solid var(--border-color);">
                      <div style="display:flex; align-items:center; gap:12px;">
                        ...
                        <div style="display:flex; flex-direction:column;">
                          <span
                            style={`color:${color}; font-weight:900; font-size:12px; text-transform:uppercase;`}
                          >
                            {name}
                          </span>
                          <span style="color:var(--text-muted); font-size:9px; font-family:monospace;">
                            ID: {id}
                          </span>
                        </div>
                      </div>
