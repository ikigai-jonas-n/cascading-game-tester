...
import { customGameOpen, setCustomGameOpen, playerId, showLoading, hideLoading } from '../../store/uiStore.js';
...
export default function CustomGameModal() {
  ...
  function openModal() {
    ...
    setJson(JSON.stringify({
      ...
      defaultRequestBody: { betAmount: 20, cashBet: '20', currencyDec: 2, stakes: [{ type: 'commonGame' }], rtpOption: 'RTP_97' },
      playerId: playerId(),
    }, null, 2));
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (!config.id || !config.name || !config.gameCode) throw new Error('Missing required fields: id, name, gameCode');
      ...
      >
        <div class="modal-content" style="max-width:560px;">
          <div class="modal-header">
            ...
            <button id="closeCustomGameBtn" class="btn-ghost" onClick={close}>×</button>
          </div>
          <div class="modal-body" style="display:flex; flex-direction:column; gap:12px;">
            ...
            <Show when={errorMsg()}>
              <div id="customGameError" style="color:var(--error); font-size:11px; padding:8px; background:rgba(244,63,94,0.1); border-radius:6px;">
                {errorMsg()}
              </div>
            </Show>
