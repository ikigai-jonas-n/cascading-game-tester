...
import { settingsOpen, setSettingsOpen, apiUrl, setApiUrl, playerId, setPlayerId } from '../../store/uiStore.js';
import { clearAllDataAndReload } from '../../services/gameService.js';
import { showDoubleGrid, setShowDoubleGrid, singleViewMode, setSingleViewMode, bypassAnimation, setBypassAnimation } from '../../store/sessionStore.js';
          ...
          <div class="modal-body" style="display:flex; flex-direction:column; gap:16px;">
            ...
            <div class="settings-group">
              ...
              <div style="display:flex; gap:6px; margin-top:6px; flex-wrap:wrap;">
                {['http://localhost:9000', 'https://letsgo-game-gs1.iki-cit.cc', 'https://letsgo-game-gs1.iki-qat.cc'].map((url) => (
                  <button
                    class="backend-preset-btn btn-ghost"
                    data-url={url}
                    style="font-size:9px; padding:4px 8px;"
                    onClick={() => { setApiUrl(url); localStorage.setItem('api_url', url); runHealthCheck(url); }}
                  >
                    {url.includes('localhost') ? 'LOCAL' : url.includes('cit') ? 'CIT' : 'QAT'}
                  </button>
                ))}
              </div>
              <div id="backendHealthStatus" style={`font-size:10px; color:${healthColor()}; margin-top:4px;`}>{healthText()}</div>
            </div>
              ...
              <div style="display:flex; flex-direction:column; gap:8px;">
                <label style="display:flex; align-items:center; gap:8px; cursor:pointer; font-size:11px; color:var(--text-muted);">
                  <input
                    ...
                    onChange={(e) => { setShowDoubleGrid(e.target.checked); localStorage.setItem('show_double_grid', e.target.checked); }}
                  />
                ...
                <div style="display:flex; align-items:center; gap:8px; font-size:11px; color:var(--text-muted);">
                  ...
                  <select
                    ...
                    onChange={(e) => { setSingleViewMode(e.target.value); localStorage.setItem('single_view_mode', e.target.value); }}
                  >
            ...
            {/* Danger Zone */}
            <div class="settings-group" style="border-top:1px solid rgba(244,63,94,0.2); padding-top:12px;">
              <label class="settings-label" style="color:var(--error);">Danger Zone</label>
              <button
                id="clearDataBtn"
                class="btn-primary"
                style="background:rgba(244,63,94,0.1); border:1px solid var(--error); color:var(--error);"
                onClick={() => { close(); clearAllDataAndReload(); }}
              >
